		...
		<div class="flex flex-col md:flex-row gap-12 md:gap-5 py-3">
			<div class="md:w-1/2">
				<label class="flex flex-col text-white uppercase">
					modx:
					<textarea v-model="inputTag" cols="30" rows="10" spellcheck="false" class="mt-2 p-2 bg-gray-600 bg-opacity-40 border-2 border-gray-200 shadow-lg text-gray-100 rounded"></textarea>
				</label>
			</div>
			<div class="md:w-1/2 relative">
				...
				<span class="flex flex-row gap-4 justify-between items-end">
					...
					<textarea v-model="tag.result" cols="30" rows="10" class="mt-2 p-2 bg-gray-600 bg-opacity-40 border-2 border-gray-200 shadow-lg text-gray-100 rounded" disabled></textarea>
				</label>
				<button @click="copy(tag.result)" class="absolute bottom-1 right-2 flex flex-1 gap-2 justify-center p-2 rounded transition-opacity bg-gray-700 text-white mb-2 opacity-30 hover:opacity-100" title="скопировать результат">
